...
const View = (props) => {
    ...
    useEffect(()=>{
        ...
        axios
        .get(`http://localhost:5000/feedback/${props.uid}/view_response/${params.feedbackId}`)
        .then(res=>{
            console.log(res)
            ...
            <div className={classes.TopicContainer}>
                ...
                <div className={classes.Head3+" "+classes.Head}>
                    <div className={classes.Sec}>
                        ...
                        <p className={classes.Responses}>{responseData.answerData.length}</p>
                    </div>
                </div>
            </div> 
                ...
                <div className={classes.HeadTitle}>
                        ...
                        <div className={classes.QuestionBox}>
                            {responseData.questionData.map((question, index)=>{
                                return <div className={classes.Que}>
                                    <p className={classes.QuestionStatement}>{question.q_statement}</p>
                                    <p onClick={()=>{setActiveSummaryIndex(index)}} className={classes.QuestionType}>{question.type === "Subjective" ? subjective(15) : objective(15)}</p>
                                    {(summary && index === activeSummaryIndex) && <div className={classes.SummaryChart}>
                                        {question.type === "Objective" ? Object.keys(summary[index]).map(key => {
                                            if(key !== "total_responses"){
                                                console.log(key, summary[index][key])
                                                return <div className={classes.SummaryEle}>
                                                    <p>{key}</p>
                                                    <div>
                                                        <p>{summary[index][key]*summary[index]["total_responses"]/100}</p>
                                                        <p>{summary[index][key]}%</p>
                                                    </div>
                                                </div>
                                            }
                                        })
                                        : <p className={classes.SummaryEle}>Subjective Question</p>
                                        }
                                        <p onClick={()=>{setActiveSummaryIndex(null)}} style={{cursor: "pointer", color: "#df5353",fontSize: "0.8rem",fontWeight: "700",textAlign: "center",padding: "2px"}}>Close</p>
                                    </div>}
                                </div>
                            })}
                        </div>
                    ...
                    <div className={classes.Answer+" "+classes.Box}>
                        ...
                        <div className={classes.AnswerContainer}>
                                ...
                                {responseData.answerData && responseData.answerData.map((answers, index1)=>{
                                    return <div key={index1} className={classes.AnswerBox}>
                                        ...
                                        <div className={classes.AnsContainer}>
                                            {answers.answer.map((val, index)=>{
                                                if(responseData.questionData[index].type === "Objective"){
                                                    return <p style={{backgroundColor: responseData.questionData[index].options[val] !== null ? "#C7FFCE" : "#FFD8C7"}} key={index} className={classes.Ans}>{responseData.questionData[index].options[val]}</p>
                                                }
                                                else if(responseData.questionData[index].type === "Subjective"){
                                                    return <p style={{backgroundColor: val !== null ? "#C7FFCE" : "#FFD8C7"}} key={index} className={classes.Ans}>{val}</p>
                                                }
                                            })}
                                        </div>
                                    </div>
