...
const ResponseAction = (props) => {
    ...
    useEffect(() => {
        ...
        if(type === "give"){
            ...
            axios
            .get(`http://localhost:5000/response/${responseId}/${props.uid}`)
            .then(res=>{
                console.log(res)
            ...
            console.log(responseId, props.uid)
            axios
            .get(`http://localhost:5000/response/${responseId}/${props.uid}/review_response`)
            .then(res=>{
                console.log(res)
                setQuestionData(res.data.questionData)
    ...
    const submitHandler = () => {
        if(isReadyToSubmit === true){
            ...
            axios
            .post(`http://localhost:5000/response/${responseId}/submit`, {answerArray: AnswerData})
            .then(res=>{
                console.log(res)
                setLoading(false)
        ...
        <div className={classes.ResponseAction}>
            {isReadyToSubmit ? 
                ...
                ? <div>
                    ...
                    {(AnswerData && QuestionData) && QuestionData.map((question, index)=>{
                        return <DisplayQ review answerValue={( AnswerData[index] !== null )&& AnswerData[index]} handlerAnswer={(value)=>setAnswerHandler(value, index)} data={question} key={index} index={index}/>
                    })}
                    </div>
                        <div className={classes.buttonSection}>
                            ...
                            <button onClick={()=>{setIsReadyToSubmit(false)}} className={classes.CancleButton}>
                                Cancle
                            </button>
                        </div>
                </div>
            ...
            <div className={classes.Backdrop}>
                <div className={classes.Content}>
                    <div className={classes.MessageHead} style={{backgroundColor: submitted.status === 200 ? "#69dc69" : (submitted.status === 400 ? "#dc697b" : "#ababab")}}>
                        {submitted.message}
                    </div>
            ...
            <div className={classes.Container}>
                ...
                <div className={classes.QuestionContainer}>
                    {(AnswerData && QuestionData) && QuestionData.map((question, index)=>{
                        return <DisplayQ review={type === "review" ? true : false} answerValue={( AnswerData[index] !== null )&& AnswerData[index]} handlerAnswer={(value)=>setAnswerHandler(value, index)} data={question} key={index} index={index}/>
                    })}
                </div>
                {type === "give"
                ? <React.Fragment>
