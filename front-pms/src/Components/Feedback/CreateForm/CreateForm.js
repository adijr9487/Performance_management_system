...
const CreateForm = (props) => {
    ...
    const createHandler = () => {
        if(finalReview){
            ...
            axios
            .post("http://localhost:5000/feedback/create", {uid: props.uid, topic: topic, questionData: QuestionList, author: props.name})
            .then((res)=>{
                console.log(res)
        ...
        <div className={classes.CreateForm}>
            ...
            {finalReview ? 
                ...
                ? <div>
                    ...
                    {(QuestionList) && QuestionList.map((question, index)=>{
                        return <DisplayQ review data={question} key={index} index={index}/>
                    })}
                    </div>
                        <div className={classes.buttonSection}>
                            <button onClick={createHandler} className={classes.SubmitButton}>
                                Submit
                            </button>
                            <button onClick={()=>{setFinalReview(false)}} className={classes.CancleButton}>
                                Cancle
                            </button>
                        </div>
                </div>
            ...
            <div className={classes.Container}>
                <div className={classes.TopicContainer}>
                    ...
                    <input value={topic ? topic : ""} onChange={topicChangeHandler} className={classes.Topic} placeholder="untitled"/>
                </div>  
                {QuestionList && QuestionList.map((question, index)=>{
                    return <DisplayQ delete={deleteHandler} key={index} index={index} testMode data={question} />
                })}
                <CreateQ add={Q_AddHandler}/>
                <button disabled={!isReadyToSubmit} onClick={createHandler} className={classes.CreateButton}>
                    Create
                </button>
